...
def eval_linear(args):
    ...
    if args.use_mnist:
        convert_to_RGB = pth_transforms.Lambda(lambda img: img.convert("RGB"))
        sharpen = pth_transforms.Lambda(lambda img: img.filter(ImageFilter.SHARPEN))
        val_transform = pth_transforms.Compose([
                pth_transforms.Compose([
                    convert_to_RGB,
                    pth_transforms.Resize(size=256),
                    sharpen,
                ]),
                pth_transforms.Compose([
        ...
        dataset_val = datasets.MNIST(args.data_path, train=False, transform=val_transform)
        ...
        dataset_train = datasets.MNIST(args.data_path, train=True, transform=train_transform)
    else:
        val_transform = pth_transforms.Compose([
                pth_transforms.Resize(256, interpolation=3),
                pth_transforms.CenterCrop(224),
